...
/// <summary>
/// OAuth Device Code Grant against id.twitch.tv — the flow meant for apps with no
/// embedded browser or local redirect server: the user visits a URL and types a
/// short code instead of us handling any redirect. See
...
public sealed class TwitchDeviceAuthenticator
{
    ...
    public async Task<DeviceCodeInfo> RequestDeviceCodeAsync(IReadOnlyList<string> scopes, CancellationToken ct = default)
    {
    ...
    /// <summary>
    /// Polls until the user approves the device code on Twitch's site, the code
    /// expires, or cancellation is requested. Twitch responds 400
    /// "authorization_pending" for every poll before approval — that's expected,
    /// not an error.
    /// </summary>
    public async Task<TwitchToken> PollForTokenAsync(DeviceCodeInfo deviceCode, IReadOnlyList<string> scopes, CancellationToken ct = default)
    {
    ...
    {
        var scopes = root.TryGetProperty("scope", out var scopeEl)
